...
void wopSP_init(void) {
	...
	trap_Cvar_Register(NULL, WOPSP_BOTSKILL, "2",
					   CVAR_ARCHIVE | CVAR_LATCH); // like g_spSkill and the addbot-parameter 1-5
}
...
static const cmdInfo_t wopSP_cmdInfos[] = {{"wopSP_startStory", wopSP_cmd_startStory},
...
qboolean wopSP_OnExitLevel(void) {
	if (wopSP_validSE(&game_currentSE)) {
		...
		if (wopSP_validSE(&game_currentSE)) {
			...
			wopSP_cmd_stopStory();
			trap_Cvar_Set("sv_killserver",
						  "1"); // FIXME: find a better way to shutdown the server ... this doesn't work correctly
		}
...
static void wopSP_SpawnBots(char *botList, int baseDelay) {
	...
	while (*p) {
...

		{
			...
			if (teamPtr) {
				if (teamPtr[1] == team_strs[1][0])
					team = 1;
				if (teamPtr[1] == team_strs[2][0])
					team = 2;
		...
		if (game_currentSE.intro[0]) {
			...
			if (!Q_stricmp(game_currentSE.intro, "cutscene"))
				...
				trap_SendConsoleCommand(EXEC_APPEND,
										va("wop_mediaview \"%s\" \"wopSP_introFinished\"\n", game_currentSE.intro));

...
static void WoPSPMenu_Event(void *ptr, int event) {
	...
	case ID_STARTSTORY:
		...
		// trap_Cmd_ExecuteText( EXEC_APPEND, va("wop_mediaview \"video:idlogo.roq\" \"wopSP_startStory
		// %s\n\"\n",wopSPmenu.currentStory));
...
void WoPSPMenu_LoadStory(const char *newStory) {
	...
	for (i = 0; i < wopSPmenu.numMenuElements; ++i) {
		...
		for (j = 0; j < wopSPmenu.numMenuElements; ++j) {
			...
			if (wopSP_compareWithNextString(wopSPmenu.storyElements[i].se.nextAfterLoos,
											&wopSPmenu.storyElements[j].se))
		...
		if (wopSPmenu.storyElements[i].alreadyWon && wopSPmenu.storyElements[i].nextWin &&
